...
inline void PrintHelp(bool userRequested)
{
    ...
    std::cout << "Usage (defaults in curly braces if you omit the switch):\n"
                ...
                "  [" << adapters::configurationArg << " <path to .silkit.yaml or .json configuration file>]\n"
                "  [" << adapters::participantNameArg << " <participant name{SilKitAdapterGenericLinuxIO}>]\n"
                "  [" << adapters::regUriArg << " silkit://<host{localhost}>:<port{8501}>]\n"
                "  [" << adapters::logLevelArg << " <Trace|Debug|Warn|{Info}|Error|Critical|Off>]\n"
                "\n"
                "SIL Kit-specific CLI arguments will be overwritten by the config file passed by " << adapters::configurationArg << ".\n";
    std::cout << "\n"
                "Example:\n"
                "sil-kit-adapter-generic-linux-io " << adapterConfigurationArg << " ./adapter/demos/DevicesConfig.yaml " << adapters::participantNameArg << " GLIO_Participant" << '\n';
