...
$scriptDir/create_adchips_run_adapter.sh &> $logDir/create_adchips_run_adapter.out &
sleep 1 # wait 1 second for the creation/existence of the .out file
timeout 30s grep -q 'Press CTRL + C to stop the process...' <(tail -f $logDir/create_adchips_run_adapter.out -n +1) || { echo "[error] Timeout reached while waiting for sil-kit-adapter-generic-linux-io to start"; exit 1; }
