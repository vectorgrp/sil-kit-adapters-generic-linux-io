...
ChardevAdapter::ChardevAdapter(SilKit::IParticipant* participant, const std::string& publisherName,
...
void ChardevAdapter::Publish(const std::size_t n)
{
    ...
    publishBuffer.insert(publishBuffer.end(), _bufferToPublisher.begin(), _bufferToPublisher.begin() + n);
