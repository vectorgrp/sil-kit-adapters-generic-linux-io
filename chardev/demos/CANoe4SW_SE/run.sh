...
if [[ -n "$canoe4sw_se_install_dir" ]]; then
	...
	#run tests
	$canoe4sw_se_install_dir/canoe4sw-se "$PSScriptRoot/Default.venvironment" -d "$PSScriptRoot/working-dir" --verbosity-level "2" --test-unit "$PSScriptRoot/testForwardChardev.vtestunit"  --show-progress "tree-element"
    exit_status=$?
else
